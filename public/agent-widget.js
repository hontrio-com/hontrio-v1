...
var _savedSession={msgs:[],sid:'',history:[],extras:[]};
try{
...
var _extras=_savedSession.extras||[];
// Salvează session la orice modificare
function persistSession(){
  try{sessionStorage.setItem(_SESS_KEY,JSON.stringify({msgs:msgs,sid:sid,extras:_extras}));}catch(e){}
}
...
'#_h_cl{margin-left:auto;background:rgba(255,255,255,.15);border:none;width:32px;height:32px;min-width:32px;min-height:32px;border-radius:50%;cursor:pointer;display:flex;align-items:center;justify-content:center;color:#fff;flex-shrink:0;transition:background .15s;padding:0;}'+
'#_h_cl:hover{background:rgba(255,255,255,.3);}'+
...
function iX(n){n=n||14;return'<svg width="'+n+'" height="'+n+'" viewBox="0 0 24 24" fill="none" stroke="white" stroke-width="2" stroke-linecap="round"><line x1="18" y1="6" x2="6" y2="18"/><line x1="6" y1="6" x2="18" y2="18"/></svg>';}
...
function renderMsg(role,text,extra,silent){
  if(!silent){msgs.push({role:role,content:text});_extras.push(extra||null);persistSession();}
...
function doWelcome(){
  ...
  if(msgs.length>0){
    ...
    msgs.forEach(function(m,idx){
      renderMsg(m.role,m.content,_extras[idx]||null,true); // true = silent, fără push în msgs din nou
    });
    return;
  }
