        ...
        <motion.form initial={{ opacity: 0, y: 8 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.27 }} onSubmit={handleSubmit} className="space-y-4">
          ...
          <div>
            ...
            {pwLen > 0 && (
              <motion.div initial={{ opacity: 0, height: 0 }} animate={{ opacity: 1, height: 'auto' }} className="flex gap-1 mt-2.5 px-1">
                {[1,2,3,4].map(i => (
                  <div key={i} className={`h-[3px] flex-1 rounded-full transition-all duration-300 ${i <= pwStrength ? pwColors[pwStrength] : 'bg-neutral-100'}`} />
                ))}
              </motion.div>
            )}
          </div>
          ...
          <motion.div whileTap={{ scale: 0.985 }} className="pt-1">
            <button type="submit" disabled={loading}
              ...
              {loading ? <Loader2 className="h-4 w-4 animate-spin" /> : <>Creeaza cont gratuit<ArrowRight className="h-4 w-4" /></>}
            </button>
          </motion.div>
        </motion.form>
