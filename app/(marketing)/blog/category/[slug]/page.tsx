...
import { BookOpen, Clock, ChevronLeft, ChevronRight, Search, SearchX, Tag } from 'lucide-react'
...
const T = {
  ro: {
    ...
    articles: 'articole',
    headingPrefix: 'Articole din categoria',
    searchPlaceholder: 'Cauta in aceasta categorie...',
    ...
    emptySub: 'Nu exista articole publicate in aceasta categorie inca.',
...
function BlogCategoryPageInner() {
  ...
  useEffect(() => {
    fetch('/api/blog/categories')
      .then((r) => r.json())
      .then((json) => {
        const list: BlogCategory[] = json.categories ?? json ?? []
  ...
  useEffect(() => {
    ...
    fetch(`/api/blog/posts?${p.toString()}`)
      ...
      .then((json) => {
        ...
        if (!category && json.posts?.[0]?.category) {
          setCategory(json.posts[0].category)
        }
      })
  ...
  const pushParams = useCallback(
    (updates: Record<string, string | undefined>) => {
      ...
      p.delete('page')
      router.push(`/blog/category/${slug}?${p.toString()}`)
    },
  ...
  const handlePage = (p: number) => {
    ...
    router.push(`/blog/category/${slug}?${ps.toString()}`)
        ...
        <div className="relative max-w-4xl mx-auto px-6">
          ...
          >
            <div className="flex items-center gap-3 mb-4 flex-wrap">
              ...
              >
                <Tag className="h-5 w-5" style={{ color: category?.color ?? '#6b7280' }} />
              </div>
              <div>
            ...
            {category?.description && (
              <p className="text-[15px] text-neutral-500 leading-relaxed max-w-2xl mb-6">
                {category.description}
              </p>
            )}
