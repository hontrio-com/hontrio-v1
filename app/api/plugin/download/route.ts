...
function buildPluginPhp(cfg: {
  ...
  o(`define( 'HONTRIO_PRODUCT_WEBHOOK_URL', '${apiBase}/api/agent/product-webhook' );`)
  ...
  o("    $product_topics = array( 'product.created' => 'Hontrio — Produs nou', 'product.updated' => 'Hontrio — Produs actualizat', 'product.deleted' => 'Hontrio — Produs șters' );")
  ...
  o('    }')
  o('    foreach ( $product_topics as $topic => $label ) {')
  o('        if ( ! empty( $existing[$topic] ) ) {')
  o('            $wh = wc_get_webhook( intval( $existing[$topic] ) );')
  o("            if ( $wh && $wh->get_id() && $wh->get_status() === 'active' ) continue;")
  o('        }')
  o('        $wh = new WC_Webhook();')
  o('        $wh->set_name( $label ); $wh->set_topic( $topic );')
  o('        $wh->set_delivery_url( HONTRIO_PRODUCT_WEBHOOK_URL );')
  o('        $wh->set_secret( HONTRIO_WEBHOOK_SECRET );')
  o("        $wh->set_status( 'active' ); $wh->set_api_version( 'wp_api_v3' );")
  o('        $wh->set_user_id( get_current_user_id() ?: 1 );')
  o('        $id = $wh->save(); if ( $id ) $existing[$topic] = $id;')
  ...
  o('    }')
  o("    $ids = get_option( 'hontrio_webhook_ids', array() ); $ok = count( $ids ) >= 5; $dash = HONTRIO_API_BASE; ?>")
