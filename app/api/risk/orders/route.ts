...
export async function GET(req: Request) {
  try {
    ...
    if (searchParams.get('status') && searchParams.get('status') !== 'all') q = q.eq('order_status', searchParams.get('status'))
...
export async function PATCH(req: Request) {
  try {
    ...
    if (!VALID.includes(order_status)) return NextResponse.json({ error: 'Invalid status' }, { status: 400 })
    const supabase = createAdminClient()
    ...
    await supabase.from('risk_orders').update({
      order_status,
      resolved_at: ['collected','refused','returned','cancelled'].includes(order_status) ? new Date().toISOString() : null,
      updated_at: new Date().toISOString(),
    }).eq('id', order_id)
    if (order.customer_id) {
      const settings = await getSettings(supabase, order.store_id)
    ...
    if (terminal.includes(order_status) && order.customer_id) {
      try {
        const { data: full } = await supabase.from('risk_orders').select('risk_flags').eq('id', order_id).single()
        const { data: cust } = await supabase.from('risk_customers').select('risk_label').eq('id', order.customer_id).single()
        const { data: s } = await supabase.from('risk_store_settings').select('ml_weights').eq('store_id', order.store_id).single()
