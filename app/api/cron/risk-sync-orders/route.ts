...
import { safeDecrypt, mapStatus, recalc, getStoreAndSettings } from '@/lib/risk/identity'
  ...
  try {
    ...
    for (const store of (stores || [])) {
      ...
      try {
        ...
        if (touched.size) {
          const settings = await getStoreAndSettings(supabase, store.id)
        ...
        results.push({ store_id: store.id, error: e.message })
      }
    }
